...
mod tests {
    ...
    fn sample_output(event_index: u32, output_index: u8, slot: u64) -> ShieldedOutputRecord {
        ShieldedOutputRecord {
            ...
            compressed_account_hash: [0xa0u8.wrapping_add(output_index); 32],
            utxo_tree: [0xb0u8.wrapping_add(output_index); 32],
            leaf_index: 10 + output_index as u64,
            tree_sequence: 100 + output_index as u64,
...
async fn persist_outputs(
    ...
    let models = outputs
        ...
        .map(|out| shielded_utxo_outputs::ActiveModel {
            ...
            compressed_output_index: Set(out.compressed_output_index as i32),
            slot: Set(out.slot as i64),
            utxo_hash: Set(out.utxo_hash.to_vec()),
            compressed_account_hash: Set(out.compressed_account_hash.to_vec()),
            utxo_tree: Set(out.utxo_tree.to_vec()),
            leaf_index: Set(out.leaf_index as i64),
            tree_sequence: Set(out.tree_sequence as i64),
