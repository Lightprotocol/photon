...
fn build_rpc_module(api_and_indexer: PhotonApi) -> Result<RpcModule<PhotonApi>, anyhow::Error> {
    ...
    module.register_async_method("getBatchAddressUpdateInfo", |rpc_params, rpc_context| async move {
        let api = rpc_context.as_ref();
        let payload = rpc_params.parse()?;
        api.get_batch_address_update_info(payload).await.map_err(Into::into)
    })?;
